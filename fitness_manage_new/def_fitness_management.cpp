...
base_management* temp_member = NULL;							//회원 구분 후 임시적으로 회원 객체를 가리킬 포인터변수
base_management* ptr_normal_member[Members]{0};			//임시 포인터변수에서 주소를 받아 값을 저장할 포인터 배열
base_management* ptr_spinning_member[Members]{0};
base_management* ptr_pt_member[Members];
base_management* ptr_pilates_member[Members];
int universal_numb = 0;								//메뉴 번호 변수
int member_Classification = 0;						//회원 구분 변수
unsigned char member_index = 0;						//배열에서 사용할 회원 인덱스 변수

//회원의 주소를 저장하는 배열을 가져오는 변수
void member_separate()
{
	if (member_Classification == NORMAL)
		ptr_normal_member[member_index] = temp_member;
	else if (member_Classification == SPINNING)
		ptr_spinning_member[member_index] = temp_member;
	else if (member_Classification == PT)
		ptr_pt_member[member_index] = temp_member;
	else 
		ptr_pilates_member[member_index] = temp_member;
}
...
void menu_fitness_management()
{
	...
	case 1:
		...
		temp_member->add_member(temp_member);
		if (member_Classification != NORMAL)	 //일반 회원이 아니라면 추가 정보 입력
			temp_member->additional_add_member(temp_member);
		break;

	case 2:
		cout << "회원 정보 수정 화면입니다." << endl << endl;
		temp_member->rewrite_member(temp_member);
		break;
	...
	case 3:
		...
		cin >> universal_numb;
		if (universal_numb == 1)
			temp_member->search_member(temp_member);
	...
	case 4:
		...
		temp_member->print_member(temp_member);
		break;

	case 5:
		cout << "회원 삭제 화면입니다." << endl << endl;
		temp_member->delete_member(temp_member);
		break;
...
//회원 추가함수 정의문
void base_management::add_member(base_management* temp_member)
{
	bool cnt = false;						//인원 초과를 확인하는 변수

	for (int i = 0; i < Members; i++)		//반복문을 통해 빈 인덱스를 회원 인덱스, 회원 번호로 지정한다.
	{
		if (ptr_normal_member[i] == 0)	 //빈 회원번호가 있다면 회원 추가
		{
			member_index = i;						
			temp_member[member_index].normal_member_number = i + 1;
			cnt = true;								
			break;
		}
	}
	...
	cout << "이름 : ";
	cin >> temp_member->normal_name;
	cout << "핸드폰 번호 (010-0000-0000): ";
	cin >> temp_member->normal_hpnumber;
	cout << "성별 (남/여) : ";
	cin >> temp_member->normal_gender;
	cout << "시작일(YYYYMMDD) : ";
	cin >> temp_member->normal_start;
	cout << "회원권 종류(개월 수 입력) : ";
	cin >> normal_memebership;
	normal_end = normal_start + normal_memebership * 30;
	cout << "운동복 대여 여부 (y/n) : ";
	cin >> normal_memebership;
	temp_member->normal_clothes = (normal_memebership == 'y') ? true : false; //삼항연산자
